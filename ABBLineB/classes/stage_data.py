...
class Run():
        ...
        for prop in target_dict:
            ...
            for key in target_dict[prop]:
                if target_dict[prop][key] not in ["", None, []]:
                    ...
                    dbData.append(row)
        if dbData != []:
            myDb["mixing_staging_lineB_"+rec_key].remove({'asofdate':self._date})
            #record_id = myDb["staging"].update(_to_push, { "$set" : _to_push}, upsert=True)
            record_id = myDb["mixing_staging_lineB_"+rec_key].insert_many(dbData)
    ...
    def download_stage_report(self, out_list, _date, rec_key):
        ...
        out_pdf= FILE_PATH + 'Report_'+selectedDate+'_Demo.pdf'
    ...
    def get_report_list(self, _date, rec_key):
        ...
        db = myDb["mixing_staging_lineB_"+str(rec_key)]
    ...
    def get_home(self, dateparam, rec_key):
        ...
        myDb = connection["analyse_db"]
        # myDb = connection["analyse_db"]
        db = myDb["mixing_staging_lineB_"+rec_key]
...
if __name__ == "__main__":
    ...
    # dbManager.get_today_records("2019-04-10")
    dbManager.read_and_download_report("2019-07-25")
