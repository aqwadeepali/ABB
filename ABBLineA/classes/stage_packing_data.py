        ...
        for prop in target_dict:
            ...
            for key in target_dict[prop]:
                if target_dict[prop][key] not in ["", None, []]:
                    ...
                    dbData.append(row)
        if dbData != []:
            myDb["packing_staging"].remove({'asofdate':self._date})
            record_id = myDb["packing_staging"].insert_many(dbData)
            print '%s records found', str(len(dbData))
            self.log.info('%s records found', str(len(dbData)))
