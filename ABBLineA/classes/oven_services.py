...
SETTING_PATH = SETTING_PATH.replace("oven_services.pyc", "")
SETTING_PATH = SETTING_PATH.replace("oven_services.pyo", "")
SETTING_PATH = SETTING_PATH.replace("oven_services.py", "")
SETTING_PATH = SETTING_PATH + "data/"

FILE_PATH = str(os.path.realpath(__file__))
FILE_PATH = FILE_PATH.replace("oven_services.pyc", "")
FILE_PATH = FILE_PATH.replace("oven_services.pyo", "")
FILE_PATH = FILE_PATH.replace("oven_services.py", "")
...
def register_services(app, WSGI_PATH_PREFIX):
    OvenServices(app, WSGI_PATH_PREFIX)
...
class OvenServices:
    ...
    def __init__(self, app, API):
        ...
        # # Home API
        self.app.add_url_rule(API + '/services/getovenhome', 'getovenhome', self.get_oven_home, methods=['POST'])
        self.app.add_url_rule(API + '/services/getovenreport', 'getovenreport', self.get_oven_report, methods=['POST'])
        self.app.add_url_rule(API + '/services/saveovenreport', 'saveovenreport', self.save_oven_report, methods=['POST'])
        self.app.add_url_rule(API + '/services/saveovenhome', 'saveovenhome', self.save_oven_home, methods=['POST'])
    ...
    def get_settings(self):
        try:
            ...
            recipeName = ""
            recipeSet = ""
            reader = csv.DictReader(open(SETTING_PATH + 'oven_settings.txt'), delimiter="\t")
            for row in reader:
    ...
    def get_oven_report(self, mode="view"):
        try:
            params = self.get_params(request)
            self.get_settings()
            result = False
            ...
            self.log.exception("get_oven_report Exception....")
            self.log.exception(e)
                

    def get_oven_home(self):
        try:
            params = self.get_params(request)
            self.get_settings()
            result = False
            ...
            if _dict != {}:
                ...
                for recKey in self.recipeKeys:
                    ...
                    for xtime in lineData:
                        ...
                        bavg = elib.tonum(bsum, "float") / elib.tonum(blen, "float")
                        plotval = elib.tonum(bavg, "float")/60
                        # avg = sum(line_data[xtime][key]) / len(line_data[xtime][key])
            ...
            return jsonify(Results = {'overview':out_result, "average": _averageBakingTime}) 
        except Exception as e:
            self.log.exception("get_oven_home Exception....")
            self.log.exception(e)


    def downloadOvenReport(self, out_report):
        try:
            params = self.get_params(request)
            ...
            self.log.exception("downloadOvenReport Exception....")
            self.log.exception(e)
    ...
    def save_oven_report(self):
        try:
            self.log.debug("Saving Report...")
            out_list = self.get_oven_report(mode="download")
            filename = self.downloadOvenReport(out_list)
            path = FILE_PATH
            uuidstr = str(uuid.uuid4())
            filepath = filename

            zip_name = "Oven_Reports_"+uuidstr+".zip"
            ...
            self.log.exception("save_oven_report Exception....")
            self.log.exception(e)
            
    def save_oven_home(self):
        try:
            self.log.debug("Saving Dashboard Report...")
            filename = self.get_oven_dashboard_report()
            path = FILE_PATH
            uuidstr = str(uuid.uuid4())
            filepath = filename
    ...
    def get_oven_dashboard_report(self):
        try:
            params = self.get_params(request)
            self.get_settings()
            result = False
            ...
            for prop in self.reportKeys:
                ...
                imgname = 'line_'+str(propname)+'.png'
                                    ...
                                    '<div class="flex flex--row flex--center flex__item margin--all">'\
                                        ...
                                        '<div class="flex flex--col flex__item" style="">'\
                                            ...
                                            '<div class="flex flex__item flex--middle flex--right text-large margin--right overview-value-border" style="">'\
                                                '<div class="flex__item text-right" style="margin-right:2px;color:white;">'+str(propData["MaxTime"])+'</div>'\
                                            '</div>'\
                                            '<div class="flex flex__item flex--middle flex--right text-large margin--right overview-value-border">'\
                                                '<div class="flex__item text-right" style="margin-right:2px;">'+str(propData["MaxTime"])+'</div>'\
                                            '</div>'\
                                            '<div class="flex flex__item flex--middle flex--right text-large margin--right overview-value-border" style="border-bottom:1px solid #e9e9e9;">'\
                                                '<div class="flex__item text-right" style="margin-right:2px;">'+str(propData["MinTime"])+'</div>'\
                                            '</div>'\
                                            '<div class="flex flex__item flex--middle margin--left margin--top">'\
                                                '<div class="flex__item text-left" style="margin-left:2px;color:white;">&#8451;</div>'\
                                            '</div>'\
                                            '<div class="flex flex__item flex--middle flex--right text-large margin--right overview-value-border" style="">'\
                                                '<div class="flex__item text-right" style="margin-right:2px;color:white;">'+str(propData["SetMaxTime"])+'</div>'\
                                            '</div>'\
                                            '<div class="flex flex__item flex--middle flex--right text-large margin--right overview-value-border">'\
                                                '<div class="flex__item text-right" style="margin-right:2px;">'+str(propData["SetMaxTime"])+'</div>'\
                                            '</div>'\
                                            '<div class="flex flex__item flex--middle flex--right text-large margin--right overview-value-border" style="border-bottom:1px solid #e9e9e9;">'\
                                                '<div class="flex__item text-right" style="margin-right:2px;">'+str(propData["SetMinTime"])+'</div>'\
                                            '</div>'\
                                        '</div>'\
                                    '</div>'\
                                '</div>'\
            ...
            # pdffile = pdfkit.from_file(intermediate_html, out_pdf,configuration=config, options=options)
            pdffile = pdfkit.from_file(intermediate_html, out_pdf,options=options)
